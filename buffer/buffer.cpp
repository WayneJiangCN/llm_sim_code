...
namespace GNN
{
  ...
  Buffer::Buffer(const std::string& _name, int num_channels, size_t capacity_per_channel)
    : SimObject(_name), num_channels_(num_channels), maxSizePerChannel_(capacity_per_channel),
  ...
  {
    D_DEBUG("BUFFER",
            "Buffer %s 初始化完成，channels=%d capacity=%zu\n",
            name().c_str(),
            num_channels_,
            maxSizePerChannel_);
  }
  ...
  bool Buffer::enqueueWrite(int channel, addr_t addr, const std::vector<storage_t>& data)
  {
  ...
  bool Buffer::enqueueWrite(int channel, PacketPtr pkt)
  {
    if (channel < 0 || channel >= num_channels_)
    {
      D_ERROR("BUFFER", "Invalid channel %d for buffer %s", channel, name().c_str());
      return false;
    }
    ...
    {
      D_WARN("BUFFER",
             "Buffer %s channel %d queue full (size=%zu)",
             name().c_str(),
             channel,
             writeQueues[channel].size());
      return false;
    }
  ...
  void Buffer::trySendWrite(int channel)
  {
    ...
    else
    {
      ...
      D_DEBUG("BUFFER", "Buffer %s channel %d 发送失败，等待重试", name().c_str(), channel);
    }
  }
  ...
  bool Buffer::recvTimingResp(int channel, PacketPtr pkt)
  {
    ...
    {
      D_WARN("BUFFER", "Buffer %s channel %d 收到意外写响应", name().c_str(), channel);
    }
    else
    {
      outstandingWrites[channel]--;
  ...
  // MemSidePort 实现
  Buffer::MemSidePort::MemSidePort(const std::string& _name, Buffer& buf, int channel)
